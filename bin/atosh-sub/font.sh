set_font() {
	...
	cp "$DOTFILES_PATH/configs/alacritty/fonts/$file_name.toml" ~/.config/alacritty/font.toml
...
if [ "$#" -gt 1 ]; then
	...
	choice=$(gum choose "UDEV Gothic" "Cascadia Mono" "Fira Mono" "JetBrains Mono" "Meslo" "> Change size" "<< Back" --height 8 --header "Choose your programming font")
fi

case $choice in
"UDEV Gothic")
	set_font "UDEVGothic Nerd Font" "
"Cascadia Mono")
...
"Meslo")
	...
	;;
"> Change size")
	source $DOTFILES_PATH/bin/atosh-sub/font-size.sh
	exit
	;;
esac

source $DOTFILES_PATH/bin/atosh-sub/menu.sh
